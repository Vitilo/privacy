...
// Las notas referencian el capítulo con un bloque ```bible.
//
// Uso:
//   node generar-notas-lectura.mjs <carpeta-destino> [--version NTV]
//
//   Sin --version: el bloque usa la versión por defecto del plugin, así una sola
//   colección sirve para todas (cambias la versión en ajustes).
//   Con  --version NTV: fija esa versión en cada bloque y hace los nombres de
//   nota únicos (sufijo " · NTV") para que dos colecciones convivan en el mismo
//   vault sin que los enlaces [[...]] choquen.
...
// --version VER (opcional): fija la versión y hace únicos los nombres de nota.
const args = process.argv.slice(2);
const vi = args.indexOf('--version');
const VERSION = vi !== -1 ? args[vi + 1] : null;
if (vi !== -1) args.splice(vi, 2);
const SFX = VERSION ? ` · ${VERSION}` : '';   // sufijo de unicidad para enlaces/archivos
const ROOT = args[0] || '/tmp/Biblia';
fs.rmSync(ROOT, { recursive: true, force: true });
fs.mkdirSync(ROOT, { recursive: true });

const pad = (c, total) => String(c).padStart(String(total).length, '0');
// Nombre de archivo/enlace de un capítulo (único por versión gracias al sufijo).
const chapBase = (name, c) => `${name} ${pad(c, CANON_BY_NAME[name])}${SFX}`;
const CANON_BY_NAME = {};
...
flat.forEach((it, gi) => {
  ...
  const idxL = `[[${it.b.name}${SFX}|${it.b.name}]]`;
...
BOOKS.forEach((b, i) => {
  ...
  const title = VERSION ? `${b.name} (${VERSION})` : b.name;
  fs.writeFileSync(path.join(folder, `${b.name}${SFX}.md`), `# ${title}\n\nCapítulos: ${links.join(' · ')}\n`);
});

// índice general
const at = BOOKS.slice(0, 39).map((b) => `[[${b.name}${SFX}|${b.name}]]`).join(' · ');
const nt = BOOKS.slice(39).map((b) => `[[${b.name}${SFX}|${b.name}]]`).join(' · ');
const idxTitle = VERSION ? `La Biblia — ${VERSION}` : 'La Biblia';
const idxNote = VERSION
  ? `Todos los capítulos se muestran en **${VERSION}** (fijado en cada nota).`
...
const idxFile = VERSION ? `Biblia ${VERSION}.md` : 'Biblia.md';
fs.writeFileSync(path.join(ROOT, idxFile),
  `# ${idxTitle}\n\n> ${idxNote}\n\n## Antiguo Testamento\n${at}\n\n## Nuevo Testamento\n${nt}\n`);

console.log(`Versión: ${VERSION || '(por defecto del plugin)'}`);
console.log(`Notas de capítulo: ${nNotes}`);
console.log(`Índices de libro: ${BOOKS.length}  +  1 índice general (${idxFile})`);
console.log(`-> ${ROOT}`);
